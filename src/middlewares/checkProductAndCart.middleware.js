...
export const checkProductAndCart = async (req = request, res = response, next) => {
    ...
    if (!product) throw error.notFoundError(`Product with id ${pid} not found`)// Manejo del error del producto
    if (!cart) throw error.notFoundError(`Cart with id ${cid} not found`) // Manejo del error del carrito
