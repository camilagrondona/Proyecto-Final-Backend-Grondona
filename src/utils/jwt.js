...
export const createToken = (user) => {
    const { _id, email } = user // Desestructuramos el id y el email del usuario, que son los que vamos a cifrar en el token
    const token = jwt.sign({ _id, email }, "codigoSecreto", { expiresIn: "1m" }) // 1er parametro: datos del usuario que vamos a cifrar / 2do parámetro: código secreto / 3er parámetro: tiempo de expiración 
    return token
}
