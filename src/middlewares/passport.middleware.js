...
export const passportCall = (strategy) => {
    return async (req = request, res = response, next) => {
        passport.authenticate(strategy, {session: false}, (err, user, info) => { // Equivale a esta lógica del passport config = if (user) return done(null, false, { message: "El usuario ya existe" }) null = error / false = user (porque en ese caso estaba en false porque no hacíamos la inscripcion del usuario por haber error) / info = message. Puede ser un mensaje o no, como se explica más abajo en el info.toString
            if(err) return next(err)
            if(!user) throw error.unauthorizedError(info.message)
            // ) // Si no existe un mensaje, que lo transforme en un string y me lo muestre 
